...
const ChanCharacter = ({ mood = 'happy', message = '안녕하세요, 주인님!' }) => {
    ...
    return (
        <div className="flex flex-col items-center gap-4">
            ...
            >
                {/* Main body - refrigerator shape */}
                <div className={`relative w-48 h-64 bg-gradient-to-b ${currentExpression.color} rounded-3xl shadow-2xl overflow-hidden`}>
                    {/* Refrigerator handle */}
                    <div className="absolute right-4 top-1/2 transform -translate-y-1/2 w-2 h-16 bg-white/40 rounded-full"></div>
                    ...
                    <div className="absolute top-16 left-0 right-0 flex flex-col items-center gap-3">
                        {/* Eyes */}
                        <div className="text-5xl">{currentExpression.eyes}</div>

                        {/* Mouth - changes based on mood */}
                        <div className="w-16 h-8 bg-white/80 rounded-full flex items-center justify-center">
                            {mood === 'happy' && <div className="w-12 h-4 border-b-4 border-gray-700 rounded-full"></div>}
                            {mood === 'worried' && <div className="w-12 h-4 border-t-4 border-gray-700 rounded-full transform rotate-180"></div>}
                            {mood === 'cold' && <div className="w-12 h-3 bg-blue-400 rounded-full"></div>}
                        </div>
                    </div>

                    {/* Cold effect - hat for frozen mood */}
                    {mood === 'cold' && (
                        <motion.div
                        ...
                        >
                            <div className="w-20 h-16 bg-red-400 rounded-t-full border-4 border-white"></div>
                            <div className="w-24 h-4 bg-white rounded-full -mt-1 ml-auto mr-auto"></div>
                            <Snowflake className="absolute top-2 right-2 text-white" size={16} />
                        </motion.div>
                    )}

                    {/* Sweating effect for worried mood */}
                    {mood === 'worried' && (
                        <>
                            <motion.div
                            ...
                            <motion.div
                                className="absolute top-14 right-8 text-2xl"
                                animate={{ y: [0, 20], opacity: [1, 0] }}
            ...
            >
                {/* Bubble pointer */}
                <div className="absolute -top-3 left-1/2 transform -translate-x-1/2 w-0 h-0 border-l-[12px] border-l-transparent border-r-[12px] border-r-transparent border-b-[12px] border-b-white/90"></div>
