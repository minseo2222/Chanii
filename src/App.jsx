...
function App() {
    ...
    const handleUpdateIngredient = async (ingredientId, updates) => {
        setInventory((prev) =>
            prev.map((item) => (item.id === ingredientId ? { ...item, ...updates } : item))
        );
        try {
    ...
    const handleUpload = async (data) => {
        ...
        if (uploadPrefill) {
            ...
            if (data.setProfile && finalImage && uploadPrefill.recipeId) {
                ...
                alert(`'${data.title}' 사진이 이 레시피의 대표 사진으로 설정됐어요.`);
            }
        }
    ...
    const handleConfirmCookingComplete = (result) => {
        ...
        if (consumption.length || purchases.length) {
            ...
            setInventory((prev) => {
                ...
                if (consumption.length) {
                    updated = updated
                        .map((item) => {
                            const match = consumption.find((c) =>
                                (item.name || '').toLowerCase().includes((c.name || '').toLowerCase())
                            );

                            if (!match || !match.units || match.units <= 0) return item;
                ...
                if (purchases.length) {
                    const addedItems = purchases.map((p) => ({
                        id: `${Date.now()}-${globalThis.crypto?.randomUUID?.() || Math.random().toString(16).slice(2)}`,
                        name: p.name,
                        icon: 'square',
                        quantity: p.quantity || '',
                        location: p.location || 'refrigerated',
                        processingState: p.processingState || '원물',
                        expiryDate: p.expiryDate || addedDate,
                        addedDate
                    }));
                ...
                consumption.forEach((c) => {
                    const target = updated.find((item) =>
                        (item.name || '').toLowerCase().includes((c.name || '').toLowerCase())
                    );
                    if (target) {
        ...
        if (recipe) {
            handleOpenUpload({
                ...
                description: `${recipe.name}을 맛있게 만들었어요. 다음에도 또 도전해볼 만한 메뉴예요.`,
        ...
        <div className="app">
            ...
            {justLeveledUp && (
                <div className="modal-overlay">
                    ...
                    >
                        <h2 className="text-2xl font-bold mb-2 bg-gradient-to-r from-pastel-purple to-pastel-blue bg-clip-text text-transparent">
                            레벨 업!
                        </h2>
                        <p className="text-gray-700 mb-4">
                            찬이가 한 단계 성장했어요. 현재 레벨은 {justLeveledUp.newLevel} 레벨입니다.
                        </p>
                        <p className="text-amber-600 font-semibold mb-6">
                            보상으로 코인 {justLeveledUp.coinsEarned}개를 받았어요.
                        </p>
                        <button
